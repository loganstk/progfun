...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    ((w.toLowerCase groupBy identity) map {case (c, list) => (c, list.length)}).toList sortBy (_._1)


  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences =
    //(s flatMap wordOccurrences) sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    if (occurrences.isEmpty) List(Nil)
      ...
      val currentCharFreq = occurrences.head._2
      (for {
        i <- 0 until (currentCharFreq + 1)
        os <- restOccurrences
      } yield if (i == 0) os else (currentChar, i) :: os).toList
    }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def iter(occs: Occurrences): List[Sentence] =
      if (occs.isEmpty) List(Nil)
        ...
        restSentence <- iter(subtract(occs, wordOccurrences(word)))
