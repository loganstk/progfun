...
trait Huffman extends HuffmanInterface {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    def insertTree(tree: Fork, restTrees: List[CodeTree]): List[CodeTree] = {
      if (restTrees.isEmpty) List(tree)
      else if (tree.weight < weight(restTrees.head)) tree :: restTrees
      else restTrees.head :: insertTree(tree, restTrees.tail)
    }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def chooseBranch(bit: Bit, currentTree: Fork): CodeTree =
      if (bit == 0) currentTree.left
      else currentTree.right
